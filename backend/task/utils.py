...
def ir_reversion(ir):
    ...
    try:
        revised['repair_type']
        try:
            lists =[(values) for key, values in revised['repair_type'].items()]
            repair_type = [(y) for k,v in lists[0] for x,y in repair_types if v==x]
            revised['repair_type'] = repair_type
        except:
            lists =[(values) for values in revised['repair_type']]
